...
def get_host_support_data(hostname: str, metric_name: Union[str, List[str]], time_from: int, time_to: int):
    ...
    if os_type:
            try:
                if os_type['data'][0]['value']:
                    os_type = os_type['data'][0]['value']
                    supporting_data['os_type'] = get_os_data(os_type)
            except:
                logger.error(f"Error parsing OS type for {hostname}: {os_type}")

    elif host_group in ['Switches','Router']:
    ...
    elif host_group in ['Firewall']:
        require_metric = {"Description":"System description","System uptime":"System uptime"}
        for key, value in require_metric:
    ...
    for metric in metric_name:
        if metric in cpu_items:
            Number_of_CPUs = zbx.get_metric_data(
                hostname=hostname,
            ...
            if Number_of_CPUs['data']:
                supporting_data['Number of CPUs'] = Number_of_CPUs['data'][0]['value']

            Number_of_cores = zbx.get_metric_data(
                hostname=hostname,
            ...
            if Number_of_cores['data']:
                supporting_data['Number of cores'] = Number_of_cores['data'][0]['value']
            ...
            if interface_name not in interfaces:
                ...
                if interface_speed_data['data']:
                    ...
                    if speed_value is not None:
                        try:
                            speed_value_int = int(speed_value)
                            speed_value = format_interface_bytes(speed_value_int)
                        except (ValueError, TypeError):
                            speed_value = f"Invalid ({speed_value})"
                        supporting_data[f'{interface_name} Speed'] = f"{speed_value}"
...
def generate_pdf_report(
    ...
    try:
        ...
        if metadata:
            support_data_items = list(metadata.items())

            # Only split metadata into parts if it's simple key-value pairs (not already nested or special)
            if len(support_data_items) > 0:
                if len(support_data_items) <= 4:
                    column_count = 3
                else:
                    column_count = 2

                metadata["supporting_data_parts"] = [
                    support_data_items[i::column_count] for i in range(column_count)
                ]
        ...
        html_content = template.render(
            ...
            user_name=user_name,
            problem_table_html=problem_table_html
            )
...
def plot_all_metrics(df, output_path="multi_series_plot.png", threshold=None, unit=None):
    ...
    try:
        ...
        max_cols_per_row = 3
        total_items = len(metrics) + (1 if threshold is not None else 0)
        legend_rows = int(np.ceil(total_items / max_cols_per_row))

        ax.legend(
            handles=custom_legend,
            loc='upper center',
            bbox_to_anchor=(0.5, -0.25 * legend_rows),  # Push lower for more rows
            frameon=False,
            fontsize=10,
            ncol=min(total_items, max_cols_per_row)
        )
        ...
        # Extend y-axis if threshold is outside range
        if threshold is not None and isinstance(threshold, (int, float)):
            current_ylim = ax.get_ylim()
            if threshold > current_ylim[1]:
                ax.set_ylim(top=threshold * 1.1)  # Add headroom above threshold
...
def generate_severity_combined_chart(df, output_path='severity_combined_chart.png'):
    ...
    severity_counts = df['severity_chart'].value_counts().reindex(severity_order, fill_value=0)
...
def generate_common_issues_chart(df, chart_path='common_issues_bar_chart.png', max_bars=10):
    """
    Generates a horizontal gradient bar chart like the one shown in your image.
    Shows event_name above each bar and uses total_count for bar length.
    """
    # Limit to top N issues
    df = df.sort_values(by='total_count', ascending=False).head(max_bars)
    event_names = df['event_name'].tolist()
    counts = df['total_count'].tolist()

    fig, ax = plt.subplots(figsize=(10, 0.5 * len(df) + 1))  # Height based on number of bars

    for i, (event, count) in enumerate(zip(event_names, counts)):
        ...
        # Draw the bar
        ax.add_patch(patches.Rectangle((0, y), count, bar_height, color=color))

        # Show event name above bar
        ax.text(0, y - 0.25, event, fontsize=9, ha='left', va='top')

        # Show value at end of bar
        ax.text(count + max(counts) * 0.01, y + bar_height / 2, str(count), fontsize=9, va='center', ha='left')

    ax.set_xlim(0, max(counts) * 1.1)
    ax.set_ylim(-0.5, len(event_names))
    ax.invert_yaxis() 
...
def main_f(time_from: int, time_to: int, hostname: Optional[List[str]] = None, metric_name: Optional[List[str]] = None, to_email: Optional[List[str]] = None, report_type = None, user_name = None):
    ...
    if report_type == 'interface':
        ...
        if metric_name[0] == 'all':
            ...
            df = d[d['name'].str.contains(r'Interface .*?(?:Bits sent|Bits received)', regex=True)]
            item = df['name']
            for i in item:
        ...
        # Prepare a dict to map interfaces to their columns
        interface_columns = {}

        for col in t[0].columns:
            if col == "clock_bin":
                continue
            m = re.search(r"Interface\s+(.*?):\s+Bits\s+(sent|received)", col, re.IGNORECASE)
            if m:
                iface = m.group(1).strip()
                interface_columns.setdefault(iface, []).append(col)

        # Create total columns per interface
        for iface, cols in interface_columns.items():
            new_col = f"{iface}_total_bits"
            t[0][new_col] = t[0][cols].sum(axis=1)
        ...
        df['acknowledged'] = df['acknowledged'].map(
            lambda x: '<div style="text-align:center;"><span style="color:green;">✔</span></div>' if x == 1 
                    else '<div style="text-align:center;"><span style="color:red;">✘</span></div>'
        )
        ...
        # Preserve original severity values for charting
        df['severity_chart'] = df['severity']

        df['severity'] = df['severity'].map(
            lambda s: f'<div style="text-align:center; color:{severity_color_map.get(s, "#000")};">{s}</div>'
        )
        ...
        df = df.drop(columns=['severity_chart'])
    ...
    elif report_type == 'unique':
        # Call your method to get common issues
        ...
        if df.empty:
            logger.warning("No Common Issues found for given arguments.")
            return "{'success': 'There is no content to return.'}", 204

        # Optionally sort or format if needed
        df = df.sort_values(by='total_count', ascending=False)

        # No statistics or metadata for now
        t = (df, None)
        meta_data = None

        # You can optionally generate a chart (e.g., top 10 common events)
        chart_path = generate_common_issues_chart(df)

        graph_path = chart_path
        subject = f"Unique Events Report"
        ...
        # Step: Get alert data and convert to HTML
        alert_df = zbx.get_alerts(
            hostname=hostname,
        ...
        # Convert Unix timestamps and durations
        if not alert_df.empty:
            for col in ['start_time', 'end_time']:
                if col in alert_df.columns:
                    alert_df[col] = alert_df[col].apply(unixtotime)

            if 'duration' in alert_df.columns:
                alert_df['duration'] = alert_df['duration'].apply(seconds_to_compact_time)

            # Drop unwanted columns
            columns_to_drop = ['trigger_name', 'eventid', 'recovery_eventid']
            alert_df.drop(columns=[col for col in columns_to_drop if col in alert_df.columns], inplace=True)

        if not alert_df.empty:
            alert_html = alert_df.to_html(index=False, classes="alert-table", border=0)
        else:
            alert_html = "<p>No problems detected for this metric.</p>"
    ...
    if not t[0].empty:
        t[0].rename(columns={'clock_bin': 'Time'}, inplace=True)
